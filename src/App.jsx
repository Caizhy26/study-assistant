...
import { fmtFull, nowDate, addDays, today, defaultTimerState } from "./utils/core";
import { StatBadge } from "./components/ui";
import { useStore } from "./hooks/useStore";
import { defaultSchedule, buildMonthlyPlan, getWeeklyPlanMeta, buildWeeklyTaskSuggestions, recommendWhatToDo, getSubjectSummary, getIntakeCompletion, getSafeTab, getActiveMainTab, mergeUniqueTasks, replanPendingTasks } from "./utils/study";
...
function buildSeedProfile() {
    ...
    return {
        ...
        subjects: [
            { id: "s1", name: "实变函数", base: "medium", weeklyHours: 8, targetScore: 88, examDate: addDays(45).toISOString().split("T")[0] },
            { id: "s2", name: "抽象代数", base: "weak", weeklyHours: 6, targetScore: 85, examDate: addDays(50).toISOString().split("T")[0] },
            { id: "s3", name: "复变函数", base: "medium", weeklyHours: 5, targetScore: 86, examDate: addDays(52).toISOString().split("T")[0] },
            { id: "s4", name: "数理统计", base: "strong", weeklyHours: 4, targetScore: 90, examDate: addDays(55).toISOString().split("T")[0] },
            { id: "s5", name: "C++ 数据结构", base: "medium", weeklyHours: 3, targetScore: 85, examDate: addDays(40).toISOString().split("T")[0] },
        ],
        schedule,
    };
}
...
const seedRecords = () => [
    { id: "r1", subject: "实变函数", minutes: 85, date: addDays(-1).toISOString().split("T")[0], note: "外测度定义 + 例 2.1" },
    { id: "r2", subject: "复变函数", minutes: 55, date: addDays(-1).toISOString().split("T")[0], note: "C-R 方程 5 题完成" },
    { id: "r3", subject: "抽象代数", minutes: 70, date: addDays(-2).toISOString().split("T")[0], note: "陪集概念，仍模糊" },
    { id: "r4", subject: "C++ 数据结构", minutes: 95, date: addDays(-2).toISOString().split("T")[0], note: "调试链表析构函数" },
    { id: "r5", subject: "数理统计", minutes: 40, date: addDays(-3).toISOString().split("T")[0], note: "MLE 例题" },
    { id: "r6", subject: "实变函数", minutes: 60, date: addDays(-3).toISOString().split("T")[0], note: "Carathéodory 条件" },
];

const seedEnergy = () => [
    { id: "e1", level: "high", date: addDays(-3).toISOString().split("T")[0], time: "09:00", note: "早晨状态好" },
    { id: "e2", level: "normal", date: addDays(-2).toISOString().split("T")[0], time: "14:00", note: "午后偏困" },
    { id: "e3", level: "high", date: addDays(-2).toISOString().split("T")[0], time: "20:00", note: "晚上效率高" },
    { id: "e4", level: "low", date: addDays(-1).toISOString().split("T")[0], time: "15:00", note: "犯困" },
...
export default function StudyAssistantHackathon() {
    ...
    const weeklyPlanMeta = useMemo(() => getWeeklyPlanMeta(profile), [profile]);
    const weeklyPreview = useMemo(() => buildWeeklyTaskSuggestions(profile, issues, currentEnergy), [profile, issues, currentEnergy]);
    ...
    const importAIPlan = () => {
        ...
        setTasks((prevTasks) => mergeUniqueTasks(prevTasks || tasks, aiTasks, { replacePendingAiImport: true, fallbackSource: "ai-import" }));
        setTab("plan");
    };

    const handleReplan = () => {
        setTasks(replanPendingTasks(tasks, profile.schedule || {}, currentEnergy));
    };
